...
export const runs = pgTable('runs', {
  ...
  level: smallint('level').notNull(), kills: integer('kills').notNull(), bossKills: integer('boss_kills').notNull(), clearTime: real('clear_time').notNull(), titan: boolean('titan').notNull().default(false),
...
export const leaderboardEntries = pgTable('leaderboard_entries', {
  pilotId: uuid('pilot_id').notNull().references(() => pilots.id, { onDelete: 'cascade' }), category: varchar('category', { length: 8 }).$type<'scores' | 'titan'>().notNull(),
